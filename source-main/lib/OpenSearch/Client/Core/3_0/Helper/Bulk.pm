...
$OpenSearch::Client::Core::3_0::Helper::Bulk::VERSION = '3.007004';
use Moo;
...
#===================================
    ...
    my $buffer  = $self->_buffer;
    my $results = try {
        my $res = $self->os->bulk( %{ $self->_bulk_args }, body => $buffer );
        $self->clear_buffer;
        return $res;
    }
    catch {
...
OpenSearch::Client::Core::3_0::Helper::Bulk - A helper module for the Bulk API
...
This class does L<OpenSearch::Client::Core::3_0::Role::Bulk> and
...
OpenSearch::Client client as the C<os> argument.
...
=item * Temporary OpenSearch errors
...
if a document with the same C<index> and C<id> already exists.
...
The response that OpenSearch returned for this action.
...
of the document currently stored in OpenSearch (if found).
...
the raw response that OpenSearch sends instead of using callbacks,
...
B<Note:> Parameters like C<index>  can be specified as C<index> or as
C<_index>, so the following two lines are equivalent:

    index => { index  => 'index',  id  => 1, source => {...}},
    index => { _index => 'index',  _id => 1, source => {...}},

B<Note:> The C<index> parameter can be specified in the
params for any action, but if not specified, will default to the C<index>
value specified in L</new()>.  This is a required parameter 
and must be specified either in L</new()> or in every action.
...
when you are using the default C<index> as set in L</new()>
...
want to delete have the default C<index> as set in L</new()>.
