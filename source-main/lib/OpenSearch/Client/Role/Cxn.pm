...
$OpenSearch::Client::Role::Cxn::VERSION = '3.007004';
use Moo::Role;
...
sub get_user_agent {
#===================================
    return sprintf("opensearch-client-perl/%s (%s; perl %s)", $OpenSearch::Client::VERSION, $^O, $]);
}
...
#===================================
    ...
    if ( $body = $self->serializer->decode($body) ) {
        ...
        $msg = $self->_munge_opensearch_exception($body) || $msg;
...
#===================================
sub _munge_opensearch_exception {
#===================================
    my ( $self, $body ) = @_;
...
OpenSearch::Client::Role::Cxn - Provides common functionality to HTTP Cxn implementations
...
L<OpenSearch::Client::Role::Cxn> provides common functionality to Cxn
implementations. Cxn instances are created by a
L<OpenSearch::Client::Role::CxnPool> implementation, using the
L<OpenSearch::Client::Cxn::Factory> class.
...
A single C<node> is passed to C<new()> by the L<OpenSearch::Client::Cxn::Factory>
...
By default, all backends that support HTTPS enable verification of
...
By default, OpenSearch nodes accept a maximum post body of 100MB or
...
If you're using the L<OpenSearch::Client::CxnPool::Sniff> module, then the
...
Enable Gzip compression of requests to and responses from OpenSearch
as follows:
...
Enable Inflate/Deflate compression of requests to and responses from OpenSearch
as follows:
...
When you use OpenSearch in production, you will probably want to reduce
...
Defaults to C<2> seconds. The L<OpenSearch::Client::CxnPool::Static> module
...
Also see L<OpenSearch::Client::Role::Cxn/default_qs_params>.
...
Processes the response received from an OpenSearch node and either
...
L<OpenSearch::Client::Transport/perform_request()>, the C<$code> is the HTTP
